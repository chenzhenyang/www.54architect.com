...
> oh-my-claudecode 核心执行模式深度解析：Autopilot、Ralph、Ultrawork、UltraQA
...
| **QA 循环** | ❌ | ❌ | ✅ 核心 | ✅ (使用 UltraQA) |
| **最大循环** | 无 | 10+ | 5 | 10+ |
| **退出条件** | 任务完成 | 用户取消/验证 | 目标达成/失败 | 所有阶段完成 |
...
| **UltraQA** | 自动 | 自动 + Architect 诊断 | 测试/构建/lint 通过 |
...
A: 不可以，两者互斥。Ralph 用于功能实现的持久化循环，UltraQA 用于 QA 循环。Autopilot 在不同阶段分别使用两者。

**Q: Ultrawork 和 Ralph 有什么区别？**  
A: Ultrawork 只提供并行执行能力，无持久化和验证。Ralph 在 Ultrawork 基础上增加了 PRD 驱动、进度持久化和强制验证。

**Q: Autopilot 的 QA 阶段和 UltraQA 一样吗？**  
A: Autopilot 的 Phase 3 直接使用 UltraQA，功能和流程完全相同。
