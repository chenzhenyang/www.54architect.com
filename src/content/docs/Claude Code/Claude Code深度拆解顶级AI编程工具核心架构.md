...
description: '深度拆解 Claude Code 的顶层架构、系统架构、执行流程、交互层、核心引擎、工具系统、上下文管理与安全权限，理解顶级AI编程工具的设计模式。'
...
categories:
  - Claude Code
original_url: https://developer.aliyun.com/article/1680327
source: 阿里云开发者
---
**简介：** Claude Code是由Anthropic开发的AI编程工具，集成于终端环境，支持代码编写、调试与项目管理。其核心功能包括自然语言交互、代码生成与优化、工具调用、上下文管理及安全控制。Claude Code通过智能处理用户指令，大幅提升开发效率，具备多模型协作与上下文压缩能力，适用于复杂项目场景。
...
![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/ea6eed76a3b04539b29f26373ac5dbcd.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)

## 2.2执行流程

![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/78faf52a2386497a8f8d388e0dbf8b55.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)
...
permission.ts
    ...
    exportconst hasPermissionsToUseTool
      // 跳过
      ...
      // 允许每个工具单独配置，由工具声明安全性，因为cc的工具都是自己实现的，他可以自己保证，后续这里MCP协议可能还需要看一看
      try {
      ...
      if (scope === 'mcprc') {
        if (process.env.NODE_ENV === 'test') {
          ...
          try {
            ...
            thrownew Error(`Failed to write to .mcprc: ${error}`)
          }
        }
      } elseif (scope === 'global') {
...
getMCPTools
    ...
    exportconst getMCPTools = memoize(async (): Promise<Tool[]> => {
...
判断命令是否有被注入
    ...
    exportconst getCommandSubcommandPrefix = memoize(
      async (
        command: string,
...
![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/11d82b4c6227493194221eccb9014abd.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)

它基于Gemini CLI改造，我们花了一些时间打磨它，融合了一些Claude Code的特性。

![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/cb0c202cb11945dc8cba9f874c5c2a02.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)

  * 支持4种运行模式。**yolo** （模型最大权限，可以执行任何操作），**accepting edits**(模型只有修改文件权限), **plan mode**(先规划后执行), **default**(模型无任何权限)；
  * 升级**subAgent** 功能。让CLI从通用助手向专家团队进化，为你提供更加专业、更加准确的建议，使用/agent可以看到更多预先配置好的agent；
  * 升级**task** 工具。有效压缩上下文长度，让CLI能够更加深入完成你的任务，上下文达到70%的时候会自动压缩；
  * 接入心流开放市场。快速安装好用的MCP工具、Subagent和自定义指令，可以通过**/mcp** 了解更多；
  * 免费使用多模态模型，也可以在CLI中粘贴图片了（Ctrl+V粘贴图片）；
  * 支持历史对话保存和回滚(**iflow --resume** 和/**chat** 命令）；
  * 支持更多更好用终端命令（iflow -h查看更多命令）；
  * 支持**VSCode、Jetbrains** 插件(注意IDE对应版本号)；
...
1\. 访问 [https://nodejs.org/zh-cn/download](https://nodejs.org/zh-cn/download?spm=a2c6h.13046898.publish-article.6.78d16ffaPuDAWA) 下载最新的 Node.js 安装程序；
...
心流开放市场帮你解决 👉 立即体验：[https://platform.iflow.cn/cli](https://platform.iflow.cn/cli?spm=a2c6h.13046898.publish-article.7.78d16ffaPuDAWA)

MCP开放市场，一键安装，解锁高效工具。

![](https://ucc.alicdn.com/pic/developer-ecology/6ibaby6qg4ku4_b2b3a6bf5643421196f22f4f7c11d6b5.gif)

agent开放市场，一键安装Subagent，引入更强大的专家agent。

![](https://ucc.alicdn.com/pic/developer-ecology/6ibaby6qg4ku4_4bd3d64dd7c84c289f5f047f5cd5360b.gif)
...
![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/46e35af2f45e4e71aefa113dc2187d3d.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)
...
## 6.3 DeepReseach
    ...
    你会根据用户的问题/需求，根据任务执行的顺序收集信息，最终以HTML的方式产出一份报告。
      ...
      - 可能包括HTML、CSS、和JavaScript文件。不断根据任务，优化这个网页项目，使网页内容逐渐完整。最终，你需要产出一个**美观、现代、易读**的"中文"可视化网页。
      # 设计要求
      **设计目标：**
      ...
      *   在根目录下维护**项目大纲**文档，来记录你的设计思路、代码模块功能、文件内容概要等，每一次变动都会记录在项目大纲中，方便下一次优化时定位优化文件。
      <用户的问题>
    计划9月15到9月24或25去西欧旅游，杭州出发亲子游，共9天。地点选择Amsterdam、Cologne、Paris和Saarbrücken。为这个行程做规划和设计，要求是： 1. 少走回头路，自驾旅游，亲子为主。 2.8月23号要在Saarbrücken参观比赛 3. Cologne预留3天 4.回城的飞机可以在巴黎或者德国，找价格低的航班。
      </用户的问题>
      当前时间是2025年8月21日，我们开始吧！

AI 代码解读

![](https://ucc.alicdn.com/6ibaby6qg4ku4/developer-article1680327/20250904/03fd57811cbb4a8c97f3bb6b2f7eeca3.webp?tmpCode=ab6f74df-e6af-4c43-b462-020103340f61&x-oss-process=image%2Fresize%2Cw_1400%2Cm_lfit%2Fformat%2Cwebp)
