---
title: Claude_Code_并行开发完全指南
lastUpdated: 2026-04-29
tags: ["Claude Code", "AI", "开发工具", "并行开发"]
categories: ["Claude Code"]
origin_title: Claude Code 并行开发完全指南：Subagents + Agent Teams + Git Worktree + 工作流编排实战
author: lkb83
skill_id: "9aa22531"
generated: "2026-04-29T22:43:50.427850"
...
| 方案 | 核心能力 | 适用场景 | 并行度 |
|------|---------|---------|--------|
| **Subagents** | 用 Markdown 定义角色指令集 | 角色分工、职责清晰 | 单实例，伪并行 |
| **Agent Teams** | 多 tmux 窗口真正并行 | 多角色同时工作 | 真并行 |
| **Git Worktree** | 多工作目录硬隔离 | 分支冲突、多人协作 | 完全隔离 |
| **工作流编排** | 计划 + 执行 + 整合流程 | 复杂项目全流程 | 编排层 |
...
|------|-------------|-------------|
| 隔离级别 | 文件系统级，完全隔离 | 会话级，共享上下文 |
| 冲突处理 | 天然避免，分支独立 | 需手动协调 |
| 适用场景 | 改动重叠、需要硬隔离 | 快速并行探索 |
| 切换成本 | 需要 cd 切换目录 | tmux 快捷键切换 |
| 学习曲线 | 低（标准 Git 命令） | 中（需了解 tmux） |
...
| 触发方式 | 说明 | 示例 |
|----------|------|------|
| **定时触发** | 按 cron 表达式定时执行 | `0 8 * * *`（每天 8:00） |
| **API 触发** | 通过 HTTP 请求触发 | 调用 Anthropic API |
| **GitHub 事件** | 监听 GitHub 事件 | PR opened、push、issue comment 等 |
...
| 架构层次 | 组件 | 职责 |
|----------|------|------|
| 调度层 | Routines | 定时/事件触发任务 |
| 编排层 | Plan + CLAUDE.md | 任务分解与流程定义 |
| 执行层 | Agent Teams | 多 agent 并行工作 |
| 隔离层 | Git Worktree | 分支硬隔离 |
| 整合层 | Routines + Agent Teams | 结果汇总与通知 |
